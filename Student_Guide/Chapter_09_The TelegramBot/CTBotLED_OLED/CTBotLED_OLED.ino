...
void setup() {
  ...
  display.clear();  
  display.drawString(0, 10, "Hello Telegram !");
...
void createInlineKB(){
  ...
  myKbd.addRow();
  myKbd.addButton("DISPLAY MESSAGE", DISPLAY_MESSAGE, CTBotKeyboardButtonQuery);
  myKbd.addRow();
...
void loop() {
  ...
  //if (myBot.getNewMessage(msg) && (msg.sender.id==AdminChatID || msg.sender.id==<<other chatID>>)) {
  if (myBot.getNewMessage(msg)) {
    ...
    if (msg.messageType == CTBotMessageText) {
      ...
      if (msg.text.equalsIgnoreCase("show keyboard")) {
        ...
        myBot.sendMessage(msg.sender.id, "Inline Keyboard", myKbd);
         ...
         if (message.length()>0){
          ...
          // write the buffer to the display
          display.display();
          myBot.sendMessage(msg.sender.id, String("Message displayed:")+message);
         }else{
          myBot.sendMessage(msg.sender.id, String("Empty Message:"));
         }
         bfSetMessage=false;
      }else {
      ...
      if (msg.callbackQueryData.equals(LIGHT_ON_CALLBACK)) {
        ...
        bfSetDuration=true;
      } else if (msg.callbackQueryData.equals(DISPLAY_MESSAGE)) {        
        myBot.sendMessage(msg.sender.id, "Please tell me your message to display:");
        bfSetMessage=true;
